(function () {
    ...
    function controller($scope, $state, $stateParams, _solver, _simulator, _bonusStats) {
        ...
        function monteCarloSimSuccess(data) {
            if (!$scope.pageState.solverStatus.logs.mc
                || data.state.hqPercent > $scope.pageState.solverStatus.state.hqPercent) {
                ...
                $scope.pageState.solverStatus.sequence = data.sequence;
        ...
        function solverProgress(data) {
            $scope.pageState.solverStatus.generationsCompleted += 1;
            $scope.pageState.solverStatus.error = null;
        }

        function solverSuccess(data) {
            $scope.pageState.solverStatus.error = null;
            if ($scope.pageState.solverStatus.generationsCompleted >= $scope.pageState.solverStatus.maxGenerations) {
